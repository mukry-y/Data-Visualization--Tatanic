...
# This plot looks really strange, because we get the density 
# within each BMI category, not within each age group!
ggplot(adult, aes (x = SRAGE_P, fill= factor(RBMI))) +
...
ggplot(adult, aes (x = SRAGE_P, fill= factor(RBMI))) +
  ...
  BMI_fill +
  facet_grid(RBMI ~ .)
# Plots 3 and 4 can be useful if we are interested in the frequency distribution
# within each BMI category.

# Plot 5 - Density histogram with position = "fill". 
#This is not an accurate representation, as density calculates the proportion across category,
# and not across bin.
ggplot(adult, aes (x = SRAGE_P, fill= factor(RBMI))) +
